...
现在，一旦得到结果，零频率分量（DC分量）将位于左上角。如果要将其置于中心位置，则需要在两个方向上将结果移动$$\frac{N}{2}$$。这只是通过函数`np.fft.fftshift()`完成的。找到频率变换后，你可以找到幅度谱。
...
所以你找到了频率变换现在你可以在频域做一些操作，比如高通滤波和重建图像，即找到逆DFT。 为此，你只需通过使用尺寸为60x60的矩形窗口进行遮罩来移除低频。 然后使用`np.fft.ifftshift()`应用反向移位，以便DC组件再次出现在左上角。 然后使用`np.ifft2()`函数找到逆FFT。 结果再次是一个复杂的数字。 你可以采取它的绝对价值。
...
OpenCV为此提供了`cv.dft()`和`cv.idft()`函数。它返回与之前相同的结果，但有两个通道。第一个通道将具有结果的实部，第二个通道将具有结果的虚部。输入图像应首先转换为np.float32。我们将看到如何做到这一点。
...
> 注意：像往常一样，OpenCV函数`cv.dft()`和`cv.idft()`比Numpy函数更快。 但是Numpy功能更加用户友好。 有关性能问题的更多详细信息，请参阅以下部分。
